    ...
    public class AutoNjsFixer
    {
        ...
        public static (float finalNjs, float finalJD, float originalJD) Calculate(float originalNJS, float originalNJO, float bpm) // Calculate AutoNjsFixerFinalNJS here since we need NJS for other patches
        {
            ...
            if (Utils.IsEnabledAutoNjsFixer())
                Plugin.Log.Info($"[AutoNjsFixer] originalNJS: {originalNJS}, originalNJO: {originalNJO}, originalJD: {originalJD} ==> finalNJS: {finalNJS}, finalJD: {finalJD} (mode: {mode})");
            else
                Plugin.LogDebug($"[AutoNjsFixer] originalNJS: {originalNJS}, originalNJO: {originalNJO}, originalJD: {originalJD} ==> (AutoNjsFixer disabled, no changes applied)"); 
